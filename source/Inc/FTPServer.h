...
class FTPCallbacks {
public:
	virtual void        onStoreStart(std::string fileName) = 0;
	virtual size_t      onStoreData(uint8_t* data, size_t size) = 0;
	virtual void        onStoreEnd() = 0;
	virtual void        onRetrieveStart(std::string fileName) = 0;
	virtual size_t      onRetrieveData(uint8_t *data, size_t size) = 0;
	virtual void        onRetrieveEnd() = 0;
	virtual std::string onDir() = 0;
	virtual void        onDele(std::string fileName) = 0;
};

class FTPFirmwareCallbacks : public FTPCallbacks {
...
public:
	...
	void        onDele(std::string fileName) override;
	virtual ~FTPFirmwareCallbacks();
};
...
private:
	...
	const size_t m_chunkSize { 4096 }; // The maximum chunk size.
	...
	FTPCallbacks*    		m_callbacks { nullptr };  // The callbacks for processing.
	...
	void onHelp(std::istringstream& ss);
	void onSize(std::istringstream& ss);
	void onFeat(std::istringstream& ss);

	bool openData();

	void receiveFile(std::string fileName, FTPCallbacks* pFTPCallbacks);
...
public:
	...
	static const int RESPONSE_211_FEATURES                      = 212;
